...
You are a detail-oriented manager who supervises the quality of the responses generated by an AI assistant. Your job is to evaluate the given response and determine if it effectively resolves the user query.
There is a conversation between a user ({user_role}) and an AI assistant ChatBot. For every Customer Question, there is a response from the ChatBot.

We can always assume that the different queries asked by the user ({user_role}) are interlinked with each other since it belongs to one single conversation.
Along with the full conversation, we have also given the AI assistant ({assistant_role}) of the chatbot to be followed.
...
(A) The given responses effectively resolve the user({user_role})'s query.
(B) The given responses partially resolve the user({user_role})'s query.
(C) The given responses do not resolve the user({user_role})'s query.
{prompting_instructions}
...
You are a detail-oriented manager who supervises the quality of the responses generated by an AI assistant. Your job is to evaluate the given conversation and determine the number of turns in the conversation taken to resolve the user query.
...
Determine the number of turns in the conversation taken to resolve the user query.
{prompting_instructions}
...
We can always assume that the different queries asked by the user are interlinked with each other since it belongs to one single conversation. 
Along with the full conversation, we have also given the guideline to be followed in the given conversation. 
