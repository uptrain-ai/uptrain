...
There is a conversation between a user ({user_persona}) and an AI assistant ChatBot. For every Customer Question, there is a response from the ChatBot.

We can always assume that the different queries asked by the user ({user_persona}) are interlinked with each other since it belongs to one single conversation.
Along with the full conversation, we have also given the AI assistant ({llm_persona}) of the chatbot to be followed.
...
(A) The given responses effectively resolve the user({user_persona})'s query.
(B) The given responses partially resolve the user({user_persona})'s query.
(C) The given responses do not resolve the user({user_persona})'s query.
{prompting_instructions}
