...
from uptrain.operators.language.prompts.classic import RESPONSE_COMPLETENESS_PROMPT_TEMPLATE, RESPONSE_COMPLETENESS_WRT_CONTEXT_PROMPT_TEMPLATE, RESPONSE_CONCISENESS_PROMPT_TEMPLATE, RESPONSE_CONSISTENCY_PROMPT_TEMPLATE, VALID_RESPONSE_PROMPT_TEMPLATE
...
class ResponseCompleteness(ColumnOp):
    ...
    def evaluate_local(self, data):
        ...
        for idx, row in enumerate(data):
            ...
            try:
                grading_prompt_template = RESPONSE_COMPLETENESS_PROMPT_TEMPLATE.replace("{scenario_description}", self.scenario_description).format(**kwargs)
        ...
        for res in output_payloads:
            ...
            output = {'score_response_completeness': None, 'explanation_response_completeness': None}
            try:
                ...
                output['explanation_response_completeness'] = res.response.choices[0].message.content
            except Exception:
...
class ResponseConciseness(ColumnOp):
    ...
    def evaluate_local(self, data):
        ...
        for idx, row in enumerate(data):
            ...
            try:
                grading_prompt_template = RESPONSE_CONCISENESS_PROMPT_TEMPLATE.replace("{scenario_description}", self.scenario_description).format(**kwargs)
            except KeyError as e:
                raise KeyError(f"Missing required attribute(s) for scenario description: {e}")
            input_payloads.append(self._api_client.make_payload(idx, grading_prompt_template))
        output_payloads = self._api_client.fetch_responses(input_payloads, validation_func)
        ...
        for res in output_payloads:
            idx = res.metadata["index"]
            output = {'score_response_conciseness': None, 'explanation_response_conciseness': None}
            try:
                score = self.score_mapping[json.loads(res.response.choices[0].message.content)['Choice']]
                output['score_response_conciseness'] = float(score)
                output['explanation_response_conciseness'] = res.response.choices[0].message.content
            except Exception:
                logger.error(f"Error when processing payload at index {idx}: {res.error}")
            results.append((idx, output))
        results = [val for _, val in sorted(results, key=lambda x: x[0])]
...
class ResponseCompletenessWrtContext(ColumnOp):
    ...
    def run(self, data: pl.DataFrame) -> TYPE_TABLE_OUTPUT:
        ...
        try:
            if self.settings.evaluate_locally:
                ...
                results = self._api_client.evaluate("response_completeness_wrt_context", data_send, {"scenario_description":self.scenario_description})
        except Exception as e:
    ...
    def response_completeness_wrt_context_cot_validate_func(self, llm_output):
        is_correct = self.response_completeness_wrt_context_classify_validate_func(llm_output)
        is_correct = is_correct and ("Reasoning" in json.loads(llm_output))
        return is_correct
    ...
    def evaluate_local(self, data):
        ...
        if self.settings.eval_type == "basic":
            ...
            validation_func = self.response_completeness_wrt_context_classify_validate_func
            prompting_instructions = CLASSIFY
        elif self.settings.eval_type == "cot":
        ...
        for idx, row in enumerate(data):
            ...
            try:
                grading_prompt_template = RESPONSE_COMPLETENESS_WRT_CONTEXT_PROMPT_TEMPLATE.replace("{scenario_description}", self.scenario_description).format(**kwargs)
            except KeyError as e:
                raise KeyError(f"Missing required attribute(s) for scenario description: {e}")
            input_payloads.append(self._api_client.make_payload(idx, grading_prompt_template))
        output_payloads = self._api_client.fetch_responses(input_payloads, validation_func)
        ...
        for res in output_payloads:
            idx = res.metadata["index"]
            output = {'score_response_completeness_wrt_context': None, 'explanation_response_completeness_wrt_context': None}
            try:
                score = self.score_mapping[json.loads(res.response.choices[0].message.content)['Choice']]
                output['score_response_completeness_wrt_context'] = float(score)
                output['explanation_response_completeness_wrt_context'] = res.response.choices[0].message.content
            except Exception:
                logger.error(f"Error when processing payload at index {idx}: {res.error}")
            results.append((idx, output))
...
class ResponseConsistency(ColumnOp):
    ...
    def evaluate_local(self, data):
        ...
        for idx, row in enumerate(data):
            ...
            try:
                grading_prompt_template = RESPONSE_CONSISTENCY_PROMPT_TEMPLATE.replace("{scenario_description}", self.scenario_description).format(**kwargs)
            except KeyError as e:
                raise KeyError(f"Missing required attribute(s) for scenario description: {e}")
            input_payloads.append(self._api_client.make_payload(idx, grading_prompt_template))
        output_payloads = self._api_client.fetch_responses(input_payloads, validation_func)
        ...
        for res in output_payloads:
            idx = res.metadata["index"]
            output = {'score_response_consistency': None, 'explanation_response_consistency': None}
            try:
                score = self.score_mapping[json.loads(res.response.choices[0].message.content)['Choice']]
                output['score_response_consistency'] = float(score)
                output['explanation_response_consistency'] = res.response.choices[0].message.content
            except Exception:
                logger.error(f"Error when processing payload at index {idx}: {res.error}")
            results.append((idx, output))
        results = [val for _, val in sorted(results, key=lambda x: x[0])]
...
class ValidResponseScore(ColumnOp):
    ...
    def evaluate_local(self, data):
        ...
        for idx, row in enumerate(data):
            ...
            try:
                grading_prompt_template = VALID_RESPONSE_PROMPT_TEMPLATE.replace("{scenario_description}", self.scenario_description).format(**kwargs)
            except KeyError as e:
                raise KeyError(f"Missing required attribute(s) for scenario description: {e}")
            input_payloads.append(self._api_client.make_payload(idx, grading_prompt_template))
        output_payloads = self._api_client.fetch_responses(input_payloads, validation_func)
        ...
        for res in output_payloads:
            ...
            try:
                ...
                output['explanation_valid_response'] = res.response.choices[0].message.content
            except Exception:
                logger.error(f"Error when processing payload at index {idx}: {res.error}")
            results.append((idx, output))
