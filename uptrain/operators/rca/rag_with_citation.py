...
class RagWithCitation(ColumnOp):
    ...
    def evaluate_local(self, data):
        question_valid_scores = ValidQuestionScore(
            col_question="question"
        ).setup(settings=self.settings).run(pl.DataFrame(data))["output"].to_dicts()

        response_valid_scores = ValidResponseScore(
            col_response="response"
        ).setup(settings=self.settings).run(pl.DataFrame(data))["output"].to_dicts()

        context_relevance_scores = ContextRelevance(
            col_question="question",
            col_context="context"
        ).setup(settings=self.settings).run(pl.DataFrame(data))["output"].to_dicts()

        factual_accuracy_scores = ResponseFactualScore(
            col_question="question",
            col_context="context",
            col_response="response"
        ).setup(settings=self.settings).run(pl.DataFrame(data))["output"].to_dicts()

        data_cited = copy.deepcopy(pl.DataFrame(data)).drop("context").rename({"cited_context": "context"})

        cited_context_relevance_scores = ContextRelevance(
            col_question="question",
            col_context="context"
        ).setup(settings=self.settings).run(data_cited)["output"].to_dicts()

        cited_factual_accuracy_scores = ResponseFactualScore(
            col_question="question",
            col_context="context",
            col_response="response"
        ).setup(settings=self.settings).run(data_cited)["output"].to_dicts()
        ...
        for idx, row in enumerate(data):
            ...
            cited_relevance = cited_context_relevance_scores[idx]['score_context_relevance']
            ...
            this_row_scores = [question_completeness, valid_response, context_relevance, factual_accuracy, cited_relevance, cited_factual]
            ...
            if question_completeness == 0:
                ...
                if context_relevance == 0:
                    ...
                    this_row_error = "Response With No Information - Poor Context Utilization"
                ...
                this_row_suggestion = "Please reach out to the UpTrain team for further brainstorming"
