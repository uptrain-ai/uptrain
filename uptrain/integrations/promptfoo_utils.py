...
def tempalate_uptrain_eval():
    ...
    results = eval_llm.evaluate(project_name = (project_name), data=data, checks=eval_type)
...
def format_uptrain_template(data, evals_compiled, threshold, eval_model, project_name):
    ...
    uptrain_template = uptrain_template.replace("(project_name)", "'{}'".format(project_name + "("+ evals_compiled["eval_type"] +")"))
