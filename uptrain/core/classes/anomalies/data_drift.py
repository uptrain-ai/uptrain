...
class DataDrift(AbstractAnomaly):
    ...
    def check(self, inputs, outputs, gts=None, extra_args={}):
        if self.is_embedding == None:
            if self.mode == "check_both":
                self.embedding_child_class.check(inputs, outputs, gts=gts, extra_args=extra_args)
                self.scalar_child_class.check(inputs, outputs, gts=gts, extra_args=extra_args)
            elif self.mode == "check_scalar_only":
                self.scalar_child_class.check(inputs, outputs, gts=gts, extra_args=extra_args)
            elif self.mode == "check_embeddings_only":
                self.scalar_child_class.check(inputs, outputs, gts=gts, extra_args=extra_args)
            else:
                feats = self.measurable.compute_and_log(inputs=inputs, outputs=outputs, gts=gts, extra=extra_args)
            ...
            self.feats = self.measurable.compute_and_log(inputs=inputs, outputs=outputs, gts=gts, extra=extra_args)
            ...
            if self.is_embedding:
                ...
                for idx in range(self.feats.shape[2]):
                    bucket_idx = np.searchsorted(self.buckets[idx], self.feats[:,:,idx])[:,0]
            ...
            if self.count < 1000:
                ...
                self.prod_dist = (self.prod_dist_counts_arr[-1] - self.prod_dist_counts_arr[int(max(self.count-2000,0)/len(extra_args["id"]))])/min(self.count,2000)
            ...
            if self.is_embedding:
                dict_emc = dict(zip(['feat_' + str(x) for x in range(self.costs.shape[0])],
                    [float(x) for x in list(self.costs)]))
                dict_emc.update({'threshold': 300})
                self.log_handler.add_scalars(self.measurable.col_name() + ' - earth_moving_costs_embedding', dict_emc, self.count, self.dashboard_name)
            else:
                dict_psi = dict(zip(['feat_' + str(x) for x in range(self.psis.shape[0])],
                    [float(x) for x in list(self.psis)]))
                dict_psi.update({'threshold': 0.3})
                self.log_handler.add_scalars(self.measurable.col_name() + ' - population_stability_index_scalar', dict_psi, self.count, self.dashboard_name)


    def is_data_interesting(self, inputs, outputs, gts=None, extra_args={}):
        if self.is_embedding == None:
            if self.mode == "check_both":
                arr1 = self.embedding_child_class.is_data_interesting(inputs, outputs, gts=gts, extra_args=extra_args)
                arr2 = self.scalar_child_class.is_data_interesting(inputs, outputs, gts=gts, extra_args=extra_args)
                return np.logical_or(np.array(arr1), np.array(arr2))
            elif self.mode == "check_scalar_only":
                return self.scalar_child_class.is_data_interesting(inputs, outputs, gts=gts, extra_args=extra_args)
            elif self.mode == "check_embeddings_only":
                return self.scalar_child_class.is_data_interesting(inputs, outputs, gts=gts, extra_args=extra_args)
            else:
            ...
            if self.save_edge_cases and self.drift_detected:
                ...
                for idx in range(self.ref_dist.shape[0]):
                    if self.is_embedding:
                        dist_from_cluster = np.sum(np.abs(self.clusters[idx][self.this_datapoint_cluster] - self.feats),axis=tuple(range(1,len(feats_shape))))/self.cluster_vars[idx][self.this_datapoint_cluster]
                    else:
                        dist_from_cluster = np.abs(np.reshape(self.clusters[idx][self.this_datapoint_cluster[idx]],-1) - self.feats[:,idx])/np.reshape(self.cluster_vars[idx][self.this_datapoint_cluster[idx]],-1)
                    for jdx in range(dist_from_cluster.shape[0]):
                        if dist_from_cluster[jdx] > 2:
                            if self.is_embedding:
                                min_dist_from_clusters = np.min(np.sum(np.abs(self.clusters[idx] - self.feats[jdx]), axis=tuple(range(1,len(feats_shape)))) / self.cluster_vars[idx])
                            else:
                                min_dist_from_clusters = np.min(np.abs(np.reshape(self.clusters[idx],-1) - self.feats[jdx,idx]) / np.reshape(self.cluster_vars[idx],-1))
                            is_interesting[jdx] = is_interesting[jdx] or (min_dist_from_clusters > 1)
    ...
    def bucket_reference_dataset(self):
        ...
        all_inputs = np.array([self.measurable.extract_val_from_training_data(x) for x in data])
