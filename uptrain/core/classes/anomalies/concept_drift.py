...
class ConceptDrift(AbstractAnomaly):
    ...
    def check(self, inputs, outputs, gts=None, extra_args={}):
        ...
        for acc in batch_acc:
            ...
            self.avg_acc = (self.avg_acc * (len(self.acc_arr)-1) + acc) / len(self.acc_arr)
            self.log_handler.add_scalars("avg_accuracy", {"avg_accuracy": self.avg_acc}, len(self.acc_arr), self.dashboard_name)
