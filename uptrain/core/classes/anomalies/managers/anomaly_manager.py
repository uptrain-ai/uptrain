...
from uptrain.core.classes.anomalies import ConceptDrift, DataDrift, CustomAnomaly, RecommendationBias, DataIntegrity, EdgeCase
...
class AnomalyManager:
    ...
    def add_anomaly_to_monitor(self, check):
        if check["type"] == Anomaly.EDGE_CASE:
            edge_case_manager = EdgeCase(
                self.fw, check["signal_formulae"])
            ...
            if "measurable_args" in check:
                drift_managers = [DataDrift(self.fw, check, is_embedding=check.get("is_embedding", None))]
            else:
                drift_managers = []
                all_feats = self.fw.feat_name_list
                for feat in all_feats:
                    check_copy = deepcopy(check)
                    check_copy.update({"measurable_args": { 
                        'type': MeasurableType.INPUT_FEATURE,
                        'feature_name': feat,
                    }})
                    drift_managers.append(DataDrift(self.fw, check_copy, is_embedding=check_copy.get("is_embedding", None)))
    ...
    def is_data_interesting(self, inputs, outputs, gts=None, extra_args={}):
        ...
        for anomaly in self.anomalies_to_check:
            if anomaly.need_ground_truth() == (gts[0] is not None):
                is_interesting.append(anomaly.is_data_interesting(inputs, outputs, gts=gts, extra_args=extra_args))
