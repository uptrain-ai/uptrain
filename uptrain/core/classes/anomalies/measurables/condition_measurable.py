...
class ConditionMeasurable(Measurable):
    ...
    def __init__(self, framework, underlying, condition) -> None:
        ...
        self.feature = FeatureMeasurable(framework, underlying["feature_name"], underlying["dictn_type"])
        if "formulae" in condition:
            ...
            raise Exception("Either func or formulae should be defined to resolve condition")
        self.condition_func = condition_func

    def _compute(self, inputs=None, outputs=None, gts=None, extra=None) -> any:
        return self.condition_func(self.feature._compute(inputs=inputs, outputs=outputs, gts=gts, extra=extra))

    def col_name(self):
        return "Measurable: Condition(" + self.feature.col_name() + "," + str(self.condition) + ")"