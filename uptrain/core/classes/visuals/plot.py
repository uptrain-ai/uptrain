...
class Plot(AbstractVisual):
    ...
    def _bar_chart(self, inputs, outputs, gts=None, extra_args={}) -> None:
        ...
        if not(self.x_feature_name in inputs.keys() or self.y_feature_name in inputs.keys()):
            return
        length = len(inputs[self.x_feature_name]) if self.x_feature_name is not None else len(inputs[self.y_feature_name])
        x_features = list(range(length)) if self.x_feature_name is None else inputs[self.x_feature_name]
        y_features = list(range(length)) if self.y_feature_name is None else inputs[self.y_feature_name]
        bars = inputs['bars']
        data = {bar: {} for bar in bars}
        for i in range(length):
            data[bars[i]].update({x_features[i]: y_features[i]})
        self.framework.log_handler.add_bar_graphs(plot_name = self.plot_name, data = data, dashboard_name = self.dashboard_name)
    ...
    def _line_chart(self, inputs, outputs, gts=None, extra_args={}) -> None:
        ...
        if not(self.x_feature_name in inputs.keys() or self.y_feature_name in inputs.keys()):
            return
        length = len(inputs[self.y_feature_name]) if self.x_feature_name is None else len(inputs[self.x_feature_name])
        x_features = list(range(length)) if self.x_feature_name is None else inputs[self.x_feature_name]
        y_features = list(range(length)) if self.y_feature_name is None else inputs[self.y_feature_name]
        for i in range(length):
            self.framework.log_handler.add_scalars(plot_name = self.plot_name, dictn = {"y_value": y_features[i]}, count = x_features[i], dashboard_name = self.dashboard_name, file_name = f'{self.x_feature_name}_{self.y_feature_name}')
