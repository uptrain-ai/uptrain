    ...
    for sub_dir in sub_dirs:
        ...
        # ######### Plotting histograms ###########
            ...
            if plot_name == "UMAP":
                ...
                if st.sidebar.checkbox(f"PCA plot"):
                    st.markdown(f"### PCA plot")
                    if model_args is not None:
                        plot_umaps(files, plot_name, sub_dir)
                    else:
                        for file in files:
...
def feat_slice(metadata):
    ...
    relevant_feat_list = st.multiselect("Select features to slice", feat_name_list)
    ...
    for i, feat in enumerate(relevant_feat_list):
        ...
        if i % 2 == 0:
            with scol1:
                vals = st.slider(
                    f"Select range for {feat}", min_val, max_val, (min_val, max_val)
                )
        else:
            with scol2:
                vals = st.slider(
