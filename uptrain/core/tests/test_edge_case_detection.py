...
def demo_signal(inputs, outputs, gts=None, extra_args={}):
    ...
    return (
        np.max(feats, axis=tuple(range(1, len(feats_shape))))
        > extra_args["val_threshold"]
    )


def test_edge_case_detection():
    # Define your signal formulae
    signal = uptrain.Signal("Demo", demo_signal, extra_args={"val_threshold": 1}) & (
        uptrain.Signal(
            uptrain.ModelSignal.CROSS_ENTROPY_CONFIDENCE, is_model_signal=True
        )
        > 0.4
    ) | (
        uptrain.Signal(
    ...
    cfg = {
        "checks": [{"type": uptrain.Anomaly.EDGE_CASE, "signal_formulae": signal}],
        "retrain": False,
    }
