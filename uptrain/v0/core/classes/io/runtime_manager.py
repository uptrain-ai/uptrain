...
class BigQueryReader:
    ...
    def run(self, time_diff):
        ...
        for var, val in self.sql_variables.items():
            run_dict.update({var: str(val + timedelta(seconds=time_diff))})
...
class RunTimeManager:
    ...
    def run_endlessly(self):
        latest_run_time = datetime.utcnow() + timedelta(
            seconds=-self.frequency_in_seconds * self.num_backlog
        )
        while 1:
            if datetime.utcnow() > (
                latest_run_time + timedelta(seconds=self.frequency_in_seconds)
            ):
                latest_run_time = latest_run_time + timedelta(
                    seconds=self.frequency_in_seconds
                )
