...
class AbstractCheck(ABC):
    def __init__(self, fw, check) -> None:
        ...
        self.measurable = MeasurableResolver(
            check.get("measurable_args", None)
        ).resolve(fw)
        ...
        if check.get("feat_slicing", False):
            self.path_dashboard_data = os.path.join(
                fw.fold_name, f"{self.dashboard_name}.csv"
            )
    ...
    def model_choices(self, check):
        allowed_model_values = [
            x["allowed_values"] for x in check.get("model_args", [])
        ]
        ...
        if num_model_options > 0:
            for m in allowed_model_values[0]:
                ...
                check_copy["model_args"].append(
                    copy.deepcopy(check_copy["model_args"][0])
                )
                del check_copy["model_args"][0]
                all_choices.extend(self.model_choices(check_copy))
            return all_choices
        else:
            self.allowed_model_values = [
                x["allowed_values"] for x in check.get("model_args", [])
            ]
