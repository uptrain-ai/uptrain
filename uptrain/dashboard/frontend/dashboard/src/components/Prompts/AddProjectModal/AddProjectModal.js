...
const AddProjectModal = (props) => {
  ...
  const handleProjectSubmit = async (event) => {
    ...
    try {
      const data = {
        ...
        metadata: metadata,
        prompt: prompt,
        prompt_name: props.promptName ? props.promptName : promptName,
      };

      const response = await fetch(
        process.env.NEXT_PUBLIC_BACKEND_URL + `api/public/add_default_prompt`,
        {
          method: "POST",
          headers: {
            "uptrain-access-token": `${uptrainAccessKey}`,
  ...
  const handleNewDataProjectSubmit = async (event) => {
    ...
    try {
      ...
      formData.append(
        "prompt_name",
        props.promptName ? props.promptName : promptName
      );
      ...
      <div className="rounded-xl border-[#5587FD] bg-[#23232D] p-8 max-w-[70%] w-full max-h-[100%] overflow-auto">
        ...
        {loading ? (
          ...
          <Step1
            ...
            prompt={prompt}
            setPrompt={setPrompt}
            selectedPromptName={props.promptName}
