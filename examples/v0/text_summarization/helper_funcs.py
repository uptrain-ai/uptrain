...
def generate_reference_dataset_with_embeddings(
    dataset, tokenizer, model, dataset_label="billsum_train"
):
    ...
    if not os.path.exists(file_name):
        for jdx in range(10):
            this_batch = [
                "summarize: " + x for x in dataset["text"][jdx * 100 : (jdx + 1) * 100]
            ]
                ...
                data.append(
                    {
                        ...
                        "num_words": get_num_words_in_text(
                            dataset[jdx * 100 + idx], None
                        ),
                    }
                )

        with open(file_name, "w") as f:
            json.dump(data, f, cls=v0.UpTrainEncoder)
    else:
...
def download_wikihow_csv_file(file_name):
    remote_url = (
        "https://oodles-dev-training-data.s3.us-west-1.amazonaws.com/" + file_name
    )
    if not os.path.exists(file_name):
        ...
        try:
            ...
            print(
                "Step 1: Paste the link https://oodles-dev-training-data.s3.amazonaws.com/wikihowAll.csv in your browser"
            )
            print(
                "Step 2: Once the csv file is downloaded, move it here (i.e. YOUR_LOC/uptrain/examples/text_summarization/"
            )
    else:
...
def get_num_words_in_text(inputs, outputs, gts=None, extra_args={}):
    txt_buckets = []
    buckets = extra_args.get(
        "buckets", [0, 200, 500, 750, 1000, 2000, 5000, 100000, 100000000]
    )
...
def get_num_prepositions_in_text(inputs, outputs, gts=None, extra_args={}):
    ...
    for txt in inputs["text"]:
        all_words = txt.split()
        num_prepositions.append(
            len(list(set(all_words).intersection(preposition_list)))
        )
    return num_prepositions


def generate_reference_dataset_with_embeddings_new(
    dataset, bert_embs, file_name, dataset_label
):
    data = []
    if not os.path.exists(file_name):
        # bert_embs_downsampled = downsample_embs(bert_embs)
