...
def convert_sentence_to_emb(sentences, device):
    ...
    model = AutoModel.from_pretrained("sentence-transformers/all-MiniLM-L6-v2").to(
        device
    )
...
model = AutoModelForSeq2SeqLM.from_pretrained("lidiya/bart-large-xsum-samsum").to(
    device
)
...
def generate_data(dataset_name, d_type, batch_size=25):
    ...
    for idx in range(int(num_points / batch_size)):
        ...
        summaries, enc_embs, bert_embs = get_summary_and_embs(
            tokenizer, model, this_batch, device
        )
